...
impl IconDirEntry {
    ...
    pub fn encode(image: &IconImage) -> io::Result<IconDirEntry> {
        ...
        let use_png = stats.has_nonbinary_alpha || image.width() * image.height() > 64 * 64;
        if use_png {
    ...
    fn encode_as_bmp_internal(image: &IconImage, stats: &ImageStats) -> io::Result<IconDirEntry> {
        let (num_colors, bits_per_pixel, data) = image.write_bmp_internal(stats)?;
        let (color_planes, bits_per_pixel) = image.cursor_hotspot().unwrap_or((1, bits_per_pixel));
    ...
    fn encode_as_png_internal(image: &IconImage, stats: &ImageStats) -> io::Result<IconDirEntry> {
        let mut data = Vec::new();
        let bits_per_pixel = image.write_png_internal(stats, &mut data)?;
        let (color_planes, bits_per_pixel) = image.cursor_hotspot().unwrap_or((0, bits_per_pixel));
        let restype = if image.cursor_hotspot().is_some() {
            ResourceType::Cursor
        } else {
            ResourceType::Icon
