use crate::ico::bmpdepth::BmpDepth;
...
impl IconImage {
    ...
    pub(crate) fn read_png_info<R: Read>(reader: R) -> io::Result<png::Reader<R>> {
    ...
    pub fn write_png<W: Write>(&self, writer: W) -> io::Result<()> {
        let _bits_per_pixel = self.write_png_internal(&self.compute_stats(), writer)?;
        Ok(())
    }
    ...
    fn write_png_internal_enc<W: Write>(
        ...
        if stats.has_alpha {
            ...
            let mut rgb_data = Vec::<u8>::with_capacity((self.rgba_data.len() / 4) * 3);
            let mut start = 0;
    ...
    pub(crate) fn read_bmp_size<R: Read>(reader: &mut R) -> io::Result<(u32, u32)> {
    ...
    pub(crate) fn read_bmp<R: Read>(mut reader: R) -> io::Result<IconImage> {
        ...
        let depth = match BmpDepth::from_bits_per_pixel(bits_per_pixel) {
            Some(depth) => depth,
            None => {
                invalid_data!("Unsupported BMP bits-per-pixel ({})", bits_per_pixel);
            }
        };
    ...
    pub(crate) fn write_bmp_internal(&self, stats: &ImageStats) -> io::Result<(u8, u16, Vec<u8>)> {
        ...
        let rgb_row_data_size = ((width as usize) * (bits_per_pixel as usize) + 7) / 8;
